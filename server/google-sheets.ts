...
class GoogleSheetsService {
  ...
  constructor() {
    this.spreadsheetId =
      ...
      "1jgi8XIWFUdu6x68oIDaGldr3s19JQC_LBZ8ZzQAsPJA";
  ...
  private parseTimestamp(timestamp: string): string {
    ...
    try {
      ...
      const shortDateMatch = cleanTimestamp.match(
      ...
      if (shortDateMatch) {
        ...
        if (!isNaN(date.getTime())) {
          console.log(
            `✅ FECHA PARSEADA: "${cleanTimestamp}" → ${date.toISOString()} (día=${day}, mes=${month}, año=${fullYear})`,
          );
          return date.toISOString();
        }
      }
      ...
      //}
      //}
      // 3. NUEVO: Formato con Barras y Hora: dd/mm/yyyy HH:mm:ss (ej: 12/12/2025 15:40:05)
      const slashDateTimeMatch = cleanTimestamp.match(
        /^(\d{1,2})\/(\d{1,2})\/(\d{4})(?:\s+(\d{1,2}):(\d{1,2})(?::(\d{1,2}))?)?$/,
      );
      if (slashDateTimeMatch) {
        const [, day, month, year, hour, minute, second] = slashDateTimeMatch;
        const date = new Date(
          parseInt(year),
          parseInt(month) - 1,
          parseInt(day),
        ...
        if (!isNaN(date.getTime())) {
          console.log(
            `✅ FECHA PARSEADA CORRECTAMENTE: "${cleanTimestamp}" → ${date.toISOString()}`,
          );
          return date.toISOString();
        }
      }
      ...
      console.warn(
        `No se pudo parsear timestamp: "${cleanTimestamp}", usando fecha actual`,
      );
      return new Date().toISOString();
    } catch (error) {
  ...
  private parseRowToLead(
    ...
    console.log(
      `📊 RAW GOOGLE SHEETS [Legacy-Fila ${rowIndex + 1}]: cliente="${row[8]}" (${typeof row[8]}) → parseado="${row[8] || ""}" (${typeof (row[8] || "")})`,
    );

    return {
      timestamp: this.parseTimestamp(row[0] || ""), // Fecha con parsing mejorado
      name: row[1] || "", // Nombre
  ...
  private async getLastRowWithData(sheetName: string): Promise<number> {
    try {
      ...
      if (!sheet?.properties?.gridProperties?.rowCount) {
        console.log(
          `⚠️ No se pudo detectar tamaño de '${sheetName}', usando límite de 5000`,
        );
      ...
      console.log(
        `📏 ${sheetName}: Detectadas ${maxRows} filas máximas en la hoja`,
      );

      // Limitar a un máximo razonable para evitar timeouts
      const safeLimit = Math.min(maxRows, 10000);
      console.log(
        `📏 ${sheetName}: Usando límite seguro de ${safeLimit} filas`,
      );
  ...
  async getSheetData(sheetName: string): Promise<SheetLead[]> {
    ...
    while (retryCount < maxRetries) {
      try {
        console.log(
          `🔄 Intentando obtener datos de '${sheetName}' (intento ${retryCount + 1}/${maxRetries})`,
        );
        ...
        console.log(
          `✅ ${sheetName}: ${validRowsCount} leads válidos, ${skippedRowsCount} filas omitidas`,
        );
        return leads;
      } catch (error: any) {
        retryCount++;
        console.error(
          `❌ Error obteniendo datos de '${sheetName}' (intento ${retryCount}):`,
          error.message,
        );

        if (retryCount >= maxRetries) {
          console.error(
            `🚨 FALLÓ después de ${maxRetries} intentos: ${sheetName}`,
          );
          return [];
        }
  ...
  async getAvailableSheetNames(): Promise<string[]> {
    try {
      ...
      console.error(
        "Error obteniendo lista de pestañas:",
        error?.message || error,
      );
  ...
  async getAllLeadsFromSheets(): Promise<SheetLead[]> {
    ...
    try {
      console.log(
        "🔄 GoogleSheetsService: Obteniendo todas las pestañas disponibles...",
      );

      // Obtener nombres de todas las pestañas disponibles
      const sheetNames = await this.getAvailableSheetNames();
      console.log(
        `📋 GoogleSheetsService: Encontradas ${sheetNames.length} pestañas para sincronizar: ${sheetNames.join(", ")}`,
      );
      ...
      for (const sheetName of sheetNames) {
        try {
          console.log(
            `🔄 GoogleSheetsService: Procesando pestaña "${sheetName}"...`,
          );
          const sheetLeads = await this.getSheetData(sheetName);
          allLeads.push(...sheetLeads);
          console.log(
            `✅ GoogleSheetsService: ${sheetLeads.length} leads obtenidos de "${sheetName}"`,
          );
        } catch (error: any) {
          console.warn(
            `⚠️ GoogleSheetsService: Error obteniendo datos de "${sheetName}":`,
            error.message,
          );
      ...
      console.log(
        `📥 GoogleSheetsService: Total de ${allLeads.length} leads obtenidos de ${sheetNames.length} pestañas`,
      );
      return allLeads;
    } catch (error: any) {
      console.error(
        "Error obteniendo todos los leads de Google Sheets:",
        error,
      );
      return [];
    }
  ...
  async getLeadsByBrandAndClient(
    marca: string,
    ...
    try {
      ...
      const marcaCapitalized =
        marca.charAt(0).toUpperCase() + marca.slice(1).toLowerCase();
      console.log(
        `🔍 FILTRO POR MARCA: Buscando en hoja "${marcaCapitalized}" para cliente "${clienteNombre}"`,
      );

      const leads = await this.getSheetData(marcaCapitalized);
      console.log(
        `📋 DATOS ACTUALES EN HOJA ${marcaCapitalized}: ${leads.length} registros totales`,
      );
      ...
      if (fechaInicio && leadsFiltered.length > 0) {
        ...
        console.log(
          `📅 FILTRO POR FECHA: ${leadsFromDate.length} leads desde ${fechaInicioStr}`,
        );
        return leadsFromDate;
      }

      console.log(
        `📊 RESULTADO FINAL: ${leadsFiltered.length} leads para ${marca} - ${clienteNombre}`,
      );
      return leadsFiltered;
    } catch (error) {
      console.error(
        `Error fetching leads for ${marca} - ${clienteNombre}:`,
        error,
      );
      return [];
    }
    ...
    try {
      ...
      const processedData = dataRows
        ...
        .map((row: any[]) => {
          ...
          const enviadosFromColumn =
            row[33] && !isNaN(Number(row[33])) ? Number(row[33]) : 0;
          const sumaDias = diasData.reduce((sum, day) => sum + day, 0);

          // Debug específico para AVEC
          if (cliente.toLowerCase().includes("avec")) {
            console.log(
              `AVEC debug: columna 33 = ${enviadosFromColumn}, suma días = ${sumaDias}`,
            );
          ...
          if (conteoRealAnalista !== null) {
            ...
            if (
              cliente.toLowerCase().includes("avec") ||
              cliente.toLowerCase().includes("grupo quijada")
            ) {
              console.log(
                `AVEC/GRUPO QUIJADA debug para cliente "${cliente}":`,
                {
                  enviadosFromColumn,
                  sumaDias,
          ...
          const pedidosPorDia =
            row[35] && !isNaN(Number(row[35])) ? Number(row[35]) : 0;
          ...
          const entregadosPorDia =
            enviados > 0 ? Math.round((enviados / diasHabiles) * 100) / 100 : 0;
          ...
          const pedidosTotal =
            row[36] && !isNaN(Number(row[36])) ? Number(row[36]) : 0;
          const numeroCampana =
            row[37] && !isNaN(Number(row[37])) ? Number(row[37]) : 1;
          const ventaPorCampana =
            row[38] && !isNaN(Number(row[38])) ? Number(row[38]) : 0;
          ...
          return {
            ...
            // Campos calculados
            porcentajeDesvio:
              pedidosPorDia > 0 && entregadosPorDia > 0
  ...
  async getClientesData(): Promise<any[]> {
    ...
    try {
      ...
      const processedData = dataRows
        ...
        .map((row: any[]) => {
          return {
            ...
            marcasSolicitadas: row[7]
              ? row[7].split(",").map((m: string) => m.trim())
              : [],
      ...
      console.log(
        `Fetched ${processedData.length} clients from Clientes sheet`,
      );
      return processedData;
    } catch (error) {
