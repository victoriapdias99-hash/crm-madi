...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ============== ENDPOINT DE TEST ESPECÍFICO FILA 413 ==============
  app.get('/api/test/row-413-chevrolet', async (req, res) => {
    try {
      console.log('🧪 INICIANDO TEST ESPECÍFICO: FILA 413 CHEVROLET');
      
      // Obtener específicamente la fila 413 de Chevrolet usando el servicio existente
      const range = 'Chevrolet!A413:I413';
      console.log(`📥 Obteniendo rango: ${range}`);
      ...
      if (!row || row.length === 0) {
        console.log('❌ Fila 413 no encontrada o vacía');
        return res.json({
          success: false,
          error: 'Fila 413 no encontrada o vacía'
        });
      }
      
      console.log('✅ Fila 413 encontrada:', row);
      
      // Extraer datos exactamente como el sistema
      const rawData = {
        timestamp: row[0] || '',
        name: row[1] || '',
        phone: row[2] || '',
        city: row[3] || '',
        modelo: row[4] || '',
        comentarioHorario: row[5] || '',
        origen: row[6] || '',
        localizacion: row[7] || '',
        cliente: row[8] || '',
      };
      
      console.log('📊 DATOS EXTRAÍDOS DE FILA 413:');
      console.log(`   Timestamp original: "${rawData.timestamp}"`);
      console.log(`   Nombre: "${rawData.name}"`);
      console.log(`   Teléfono: "${rawData.phone}"`);
      console.log(`   Cliente: "${rawData.cliente}"`);
      
      // Probar parseTimestamp usando el método del sistema
      let parsedTimestamp = '';
      try {
        parsedTimestamp = googleSheetsService['parseTimestamp'](rawData.timestamp);
        console.log(`🕒 Fecha parseada por sistema: "${parsedTimestamp}"`);
      } catch (error) {
        console.error('❌ Error en parseTimestamp del sistema:', error);
        parsedTimestamp = 'ERROR: ' + error.message;
      }
      
      const result = {
        success: true,
        test: 'FILA 413 CHEVROLET - ANÁLISIS COMPLETO',
        rowNumber: 413,
        sheetName: 'Chevrolet',
        rawRow: row,
        extractedData: rawData,
        timestampTest: {
          original: rawData.timestamp,
          parsed: parsedTimestamp,
          isValid: !parsedTimestamp.includes('ERROR'),
          dateInterpretation: parsedTimestamp.includes('ERROR') ? null : {
            year: new Date(parsedTimestamp).getFullYear(),
            month: new Date(parsedTimestamp).getMonth() + 1,
            day: new Date(parsedTimestamp).getDate(),
            hour: new Date(parsedTimestamp).getHours(),
            minute: new Date(parsedTimestamp).getMinutes()
          }
        }
      };
      
      console.log('🎯 RESULTADO FINAL FILA 413:', JSON.stringify(result.timestampTest, null, 2));
      
      res.json(result);
      
    } catch (error) {
      console.error('❌ ERROR en test fila 413:', error);
      res.status(500).json({
        success: false,
        error: error.message
      });
