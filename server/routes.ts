...
import {
  calculateDatosEnviadosPercentage,
  calculateFaltantesAEnviar,
  calculatePorcentajeDesvio,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  wss.on("connection", (ws: WebSocketWithData) => {
    ...
    console.log(
      "🔗 Nueva conexión WebSocket. Total conectados:",
      dashboardConnections.size,
    );
      ...
      try {
        ...
        switch (message.type) {
          ...
          case "register_dashboard_listener":
            ...
            console.log(
              "🔔 Cliente registrado para actualizaciones del dashboard",
            );
            ...
            ws.send(
              JSON.stringify({
                ...
                message:
                  "Registrado para recibir actualizaciones del dashboard",
              }),
            );
            break;
    ...
    ws.on("close", () => {
      ...
      console.log(
        "🔗 Conexión WebSocket cerrada. Total conectados:",
        dashboardConnections.size,
      );
    });
  });
  ...
  async function getRealtimeStats() {
    const [leadsCount, totalSpend, conversionRate, costPerLead] =
      await Promise.all([
  ...
  function convertSheetLeadToDbLead(
    ...
    return {
      ...
      cost: sheetLead.cost
        ? (
            parseFloat(sheetLead.cost.replace(/[^0-9.-]/g, "")) * 1400
          ).toString()
        : "0", // Convertir USD a ARS (aprox 1400 pesos por dólar)
      leadDate:
        sheetLead.timestamp && !isNaN(new Date(sheetLead.timestamp).getTime())
          ? new Date(sheetLead.timestamp)
          : new Date(),
    };
  }
      ...
      leads.forEach((lead) => {
        if (lead.source === "google_sheets" && lead.campaignName) {
          ...
          if (brand.includes("fiat"))
            brandCounts["Fiat"] = (brandCounts["Fiat"] || 0) + 1;
          else if (brand.includes("peugeot"))
            brandCounts["Peugeot"] = (brandCounts["Peugeot"] || 0) + 1;
          else if (brand.includes("toyota"))
            brandCounts["Toyota"] = (brandCounts["Toyota"] || 0) + 1;
          else if (brand.includes("chevrolet"))
            brandCounts["Chevrolet"] = (brandCounts["Chevrolet"] || 0) + 1;
          else if (brand.includes("renault"))
            brandCounts["Renault"] = (brandCounts["Renault"] || 0) + 1;
          else if (brand.includes("citroen"))
            brandCounts["Citroen"] = (brandCounts["Citroen"] || 0) + 1;
  ...
  async function processCampaignRecord(
    ...
    // Contar duplicados
    const duplicadosResult = await contarDuplicadosPorCampana(
      campana,
      clienteData,
      db,
      opLeadsRep,
    ...
    const tieneCampanaAnterior = await tieneCampanaAnteriorAbierta(
      campana,
      todasLasCampanas,
    );
    ...
    const clienteNombreReal =
      clienteData?.nombreCliente ||
      `${campana.marca.toUpperCase()} ${campana.numeroCampana}`;

    // Calcular métricas
    const cantidadSolicitados = campana.cantidadDatosSolicitados;
    const percentageResult = calculateDatosEnviadosPercentage(
      enviadosFinales,
      cantidadSolicitados,
    );
    const porcentajeDatosEnviados = percentageResult.percentage;
    const faltantesAEnviar = calculateFaltantesAEnviar(
      enviadosFinales,
      cantidadSolicitados,
    );
    ...
    const fechaInicio = campana.fechaCampana
      ? new Date(campana.fechaCampana)
      : new Date();
    const fechaReferencia = isPending
      ? new Date() // Para pendientes siempre es hoy
      : fechaFinExacta && fechaFinExacta !== null
        ? new Date(fechaFinExacta)
        : new Date();
    const diasTranscurridos = Math.max(
      1,
      Math.ceil(
        (fechaReferencia.getTime() - fechaInicio.getTime()) /
          (1000 * 60 * 60 * 24),
      ),
    );
    const entregadosPorDia = tieneCampanaAnterior
      ? "-"
      : enviadosFinales / diasTranscurridos;

    // Calcular inversiones
    const inversionRealizada = tieneCampanaAnterior
      ? "-"
      : enviadosFinales * (storedCpl || 0);
    const inversionPendiente = tieneCampanaAnterior
      ? "-"
      : faltantesAEnviar * (storedCpl || 0);
    ...
    return {
      ...
      faltantes: tieneCampanaAnterior
        ? "-"
        : Math.max(0, campana.cantidadDatosSolicitados - enviadosFinales),
      entregadosPorDia,
      inversionRealizada,
      inversionPendiente,
      estado,
  ...
  async function tieneCampanaAnteriorAbierta(
    campanaActual: any,
    todasLasCampanas: any[],
  ): Promise<boolean> {
    try {
      ...
      const campanasAnteriores = todasLasCampanas.filter((c) => {
        ...
        return (
          ...
          !c.fechaFin
        ); // Sin fecha de fin (campaña abierta)
      });
  ...
  app.get("/api/dashboard/datos-diarios-db-optimized", async (req, res) => {
    try {
      ...
      const { count, sql, eq, and, inArray, gte, lte, ilike } = await import(
        "drizzle-orm"
      );

      console.log(
        "🚀 [OPTIMIZED] Iniciando carga optimizada de datos diarios...",
      );
      ...
      const campanasConClientes = await db
        ...
        .leftJoin(
          clientesTable,
          eq(campanasComerciales.clienteId, clientesTable.id),
        );

      console.log(
        `📊 [OPTIMIZED] ${campanasConClientes.length} campañas obtenidas en ${Date.now() - startTime}ms`,
      );
      ...
      if (campanasFinalizadas.length > 0) {
        ...
        console.log(
          `✅ [OPTIMIZED] Leads finalizados contados: ${leadsFinalizados.length} campañas`,
        );
      }
      ...
      for (const campana of campanasEnProceso) {
        try {
          ...
          console.error(
            `Error contando leads para campaña ${campana.campanaId}:`,
            error,
          );
      ...
      console.log(
        `✅ [OPTIMIZED] Leads en proceso contados: ${campanasEnProceso.length} campañas`,
      );
      ...
      if (campanasFinalizadas.length > 0) {
        ...
        console.log(
          `✅ [OPTIMIZED] Duplicados contados: ${campanasFinalizadas.length} campañas`,
        );
      }
      ...
      for (const campana of campanasConClientes) {
        try {
          ...
          const totalDuplicados =
            duplicadosCountMap.get(campana.campanaId) || 0;
          ...
          const record = {
            ...
            faltantes: tieneCampanaAnterior
              ? "-"
              : Math.max(0, campana.cantidadDatosSolicitados - enviadosFinales), // Faltantes = Pedidos Total - Enviados
            entregadosPorDia: (() => {
              ...
              return tieneCampanaAnterior
                ? "-"
                : enviadosFinales / diasTranscurridos;
            })(), // Entregados por día = enviados / días transcurridos
            // Campos calculados adicionales
            inversionRealizada: tieneCampanaAnterior
              ? "-"
              : enviadosFinales * (storedCpl || 0),
            inversionPendiente: tieneCampanaAnterior
              ? "-"
              : faltantesAEnviar * (storedCpl || 0),
          ...
          console.error(
            `Error procesando campaña ${campana.numeroCampana}:`,
            campaignError,
          );
        }
      }
      ...
      console.log(
        `🎉 [OPTIMIZED] Procesamiento completado en ${totalTime}ms (${(totalTime / 1000).toFixed(2)}s)`,
      );
      console.log(
        `📊 [OPTIMIZED] Total campañas procesadas: ${processedData.length}`,
      );
  ...
  app.get("/api/dashboard/datos-diarios-db", async (req, res) => {
    try {
      ...
      const { db } = await import("./db");
      const { opLeadsRep, campanasComerciales } = await import(
        "../shared/schema"
      );
      ...
      console.log(
        `✅ [OPTIMIZED] ${clientes.length} clientes cargados en memoria`,
      );
      ...
      for (const campana of campanas) {
        ...
        if (clienteData) {
          const clienteIdentificador = `${campana.marca.toUpperCase()} ${campana.numeroCampana}`;
          const uniqueKey = `${clienteIdentificador}-${campana.numeroCampana}`;
          const hash = (storage as any).hashString
            ? (storage as any).hashString(uniqueKey)
            : 0;
      ...
      console.log(
        `✅ [OPTIMIZED] ${cplsMap.size} CPLs cargados en memoria (de ${allCplsData.length} registros)`,
      );
      ...
      const totalTime = Date.now() - startTime;
      console.log(
        `⏱️ [PERFORMANCE] Datos diarios cargados en ${totalTime}ms (${(totalTime / 1000).toFixed(2)}s) - ${processedData.length} campañas`,
      );
  ...
  app.get("/api/dashboard/campanas-pendientes", async (req, res) => {
    try {
      ...
      if (campanasCache && now - cacheTimestamp < CACHE_TTL) {
        ...
        console.log(
          `💾 [${requestId}] Usando caché (${cacheAge}ms antiguo) - ${campanasCache.length} campañas`,
        );
        return res.json(campanasCache);
      }

      console.log(
        `📋 [${requestId}] Cargando campañas pendientes (caché expirado o vacío)...`,
      );
      ...
      console.log(
        `📊 [${requestId}] ${campanasPendientes.length} campañas pendientes de ${campanas.length} totales`,
      );
      ...
      const processedDataPromises = campanasPendientes.map(async (campana) => {
        try {
          ...
          console.error(
            `❌ Error campaña ${campana.numeroCampana}:`,
      ...
      const processedData = (await Promise.all(processedDataPromises)).filter(
        Boolean,
      );

      const totalTime = Date.now() - startTime;
      console.log(
        `✅ [${requestId}] Completado en ${totalTime}ms - ${processedData.length} campañas`,
      );
  ...
  app.get("/api/dashboard/campanas-finalizadas", async (req, res) => {
    try {
      ...
      console.log(
        `📊 [${requestId}] ${campanasFinalizadas.length} campañas finalizadas de ${campanas.length} totales`,
      );
  ...
  app.get("/api/dashboard/datos-diarios", async (req, res) => {
    try {
      ...
      for (const campana of campanasComerciales) {
        ...
        const fechaInicioCampana = campana.fechaCampana
          ? new Date(campana.fechaCampana)
          : new Date();
        ...
        for (let i = 0; i < datosDiarios.length; i++) {
          ...
          const esMatch = clientMatcher
            ? clientMatcher.isMatch(nombreClienteBajo, clienteBajo)
            : false;
        ...
        console.log(
          `Campaign ${campana.numeroCampana} - Found ${datosParaCampana.length} matching data records for ${cliente.nombreCliente} from ${campana.fechaCampana}`,
        );

        // Debug para ver qué datos están disponibles
        if (datosParaCampana.length === 0) {
          console.log(
            `  No matches found for '${cliente.nombreCliente}' or '${campana.marca}'`,
          );
          console.log(
            `  Sample data names: ${datosDiarios
              .slice(0, 3)
              .map((d) => d.cliente)
              .join(", ")}`,
          );
          ...
          if (cliente.nombreCliente.toLowerCase().includes("grupo quijada")) {
            console.log(
              `  GRUPO QUIJADA Debug - Available data clients:`,
              datosDiarios
                .map((d) => d.cliente)
                .filter(
                  (c) =>
                    c.toLowerCase().includes("grupo") ||
                    c.toLowerCase().includes("quijada") ||
                    c.toLowerCase().includes("avec"),
                ),
            );
          }
        } else if (
          cliente.nombreCliente.toLowerCase().includes("grupo quijada")
        ) {
          console.log(
            `    GRUPO QUIJADA found data:`,
            datosParaCampana.map((d) => ({
              cliente: d.cliente,
              fecha: d.fecha,
              cantidad: d.cantidad,
        ...
        const datosAcumuladosAnteriores = campanasAnteriores.reduce(
          (suma, c) => suma + (c.cantidadDatosSolicitados || 0),
          0,
        );
        const rangoInicio = datosAcumuladosAnteriores + 1; // La campaña actual empieza después de las anteriores
        const rangoFin =
          datosAcumuladosAnteriores + campana.cantidadDatosSolicitados;
        ...
        const campanasPosteriores = campanasComerciales.filter(
          (c) =>
            c.clienteId === campana.clienteId &&
            c.marca === campana.marca &&
            c.zona === campana.zona &&
        ...
        if (datosRealesTotal <= datosAcumuladosAnteriores) {
          ...
          datosRealesTotal >= rangoFin &&
          campanasPosteriores.length > 0
        ) {
        ...
        if (
          ...
          cliente.nombreCliente.toLowerCase().includes("pamela")
        ) {
          console.log(
            `🔍 NOVO GROUP DEBUG: fechaFinReal final = ${fechaFinReal}, campana.fechaFin = ${campana.fechaFin}`,
          );
        }
        ...
        if (
          cliente.nombreCliente.toLowerCase().includes("fiat") &&
          campana.numeroCampana === "2"
        ) {
          console.log(`  TOTALES CALCULADOS:`);
          console.log(`  datosAcumulados: ${datosAcumulados}`);
          console.log(
            `  cantidadSolicitados: ${campana.cantidadDatosSolicitados}`,
          );
          console.log(
            `  porcentaje: ${(datosAcumulados / campana.cantidadDatosSolicitados) * 100}%`,
          );
        }
        ...
        if (datosAcumulados >= campana.cantidadDatosSolicitados) {
          ...
          const fechaCreacion = campana.fechaCreacion
            ? new Date(campana.fechaCreacion)
            : campana.createdAt
              ? new Date(campana.createdAt)
              : new Date();
          const horasDesdeCreacion =
            (Date.now() - fechaCreacion.getTime()) / (1000 * 60 * 60);
        ...
        if (cliente.nombreCliente.toLowerCase().includes("toyota")) {
          ...
          console.log(
            `  datosAcumuladosAnteriores: ${datosAcumuladosAnteriores}`,
          );
        ...
        */
        ...
        if (
          ...
          console.log(
            `🔍 *** NUEVO PROCESO CITROËN AMBA - Filtrado correcto por hoja de marca ***`,
          );
          ...
          try {
            const fechaInicioCampana = campana.fechaCampana
              ? new Date(campana.fechaCampana)
              : new Date();
            const leadsEspecificos =
              await googleSheetsService.getLeadsByBrandAndClient(
                campana.marca,
                cliente.nombreCliente,
                fechaInicioCampana,
              );
            ...
            console.log(
              `📊 RESULTADO AUTOMÁTICO: ${datosRealesPorHojaMarca} datos encontrados en hoja ${campana.marca} para ${cliente.nombreCliente} desde ${campana.fechaCampana}`,
            );
            ...
            console.log(
              `🎯 CONTABILIZACIÓN EXACTA: datosRealesTotal=${datosRealesTotal}, datosAcumuladosAnteriores=${datosAcumuladosAnteriores}, datosFinales=${datosFinales}`,
            );
          } catch (error) {
            ...
            console.log(
              `📊 FALLBACK CITROËN AMBA: Usando cálculo automático ${datosFinales}`,
            );
          }
        }
        ...
        if (
          cliente.nombreCliente.toLowerCase().includes("toyota") &&
        ...
        if (
          ...
          console.log(
            `🔍 FIAT AUTOS DEL SOL: Aplicando conteo real de 975 datos medidos manualmente`,
          );
          console.log(
            `🔍 Total datos "Autos del Sol": ${autosDelSolLeadsTotal} leads (medición manual del usuario)`,
          );
        ...
        const percentageResult = calculateDatosEnviadosPercentage(
          datosFinales,
          campana.cantidadDatosSolicitados,
        );
        const porcentajeDatosEnviados = percentageResult.percentage;
        const faltantesAEnviar = calculateFaltantesAEnviar(
          datosFinales,
          campana.cantidadDatosSolicitados,
        ); // Pedidos Total - Enviados
        // Obtener valores almacenados para esta campaña específica usando clienteNombre y numeroCampana
        const storedCpl = await storage.getCplByClienteAndCampana(
          cliente.nombreCliente,
          campana.numeroCampana,
        );
        const storedVenta = await storage.getVentaPorCampanaByClienteAndCampana(
          cliente.nombreCliente,
          campana.numeroCampana,
        );
        const storedPedidos = await storage.getPedidosPorDiaByClienteAndCampana(
          cliente.nombreCliente,
          campana.numeroCampana,
        );
        ...
        if (cliente.nombreCliente.toLowerCase().includes("grupo quijada")) {
          console.log(
            `🔍 DEBUG AVEC ${campana.marca}: cantidadDatosSolicitados=${campana.cantidadDatosSolicitados}, pedidosTotal=${pedidosTotal}, datosFinales=${datosFinales}`,
          );
        }
        ...
        const entregadosPorDiaPromedio =
          datosFinales > 0
            ? Math.round((datosFinales / diasHabilesMes) * 100) / 100
            : 0;
        const pedidosPorDiaCalculado =
          pedidosPorDiaReal > 0
            ? pedidosPorDiaReal
            : pedidosTotal > 0
              ? Math.round((pedidosTotal / diasHabilesMes) * 100) / 100
              : 0;

        // Calcular % de desvío: Pedidos/día entre Entregados/día
        const porcentajeDesvio = calculatePorcentajeDesvio(
          pedidosPorDiaCalculado,
          entregadosPorDiaPromedio,
        );
        ...
        try {
          ...
          if (metaAdsService && datosFinales > 0) {
            ...
            if (campana.marca.toLowerCase().includes("peugeot")) {
              ...
              campana.marca.toLowerCase().includes("citroen") ||
              campana.marca.toLowerCase().includes("citroën")
            ) {
              campaignSearchName = "Citroen";
            }

            // Calcular CPA
            cpaValue = await metaAdsService.calculateCPA(
              campaignSearchName,
              dateRange,
              datosFinales,
            );
          ...
          console.error(
            `Error calculating CPA for ${cliente.nombreCliente}:`,
            error,
          );
          cpaValue = 0;
        }
        ...
        console.log(
          `✅ MAPPING: Added campaign ${campana.numeroCampana} for client ${cliente.nombreCliente}`,
        );
        mappedData.push({
      ...
      if (mappedData.length === 0) {
        console.log(
          "No commercial campaigns found, using default Google Sheets data",
        );
        ...
        console.log(
          `Processed ${enrichedData.length} default records for datos diarios dashboard`,
        );
        return res.json(enrichedData);
      }

      console.log(
        `Processed ${mappedData.length} campaigns for datos diarios dashboard`,
      );
      res.json(mappedData);
    } catch (error) {
  ...
  app.post("/api/dashboard/mapear-campanas", async (req, res) => {
    try {
      ...
      for (const campana of campanasComerciales) {
        ...
        console.log(
          `Processing Campaign ${campana.numeroCampana} - ${cliente.nombreCliente} - ${campana.marca}`,
        );
        console.log(
          `Campaign start date: ${campana.fechaCampana}, requested data: ${campana.cantidadDatosSolicitados}`,
        );
        ...
        const datosRelacionados = datosDiarios.filter((dato) => {
          ...
          const clienteCoincide =
            dato.cliente
              .toLowerCase()
              .includes(cliente.nombreCliente.toLowerCase()) ||
            dato.clienteNombre
              .toLowerCase()
              .includes(cliente.nombreCliente.toLowerCase()) ||
        ...
        const datosOrdenados = datosRelacionados.sort(
        ...
        for (const dato of datosOrdenados) {
          ...
          const datosDelDia =
            dato.totalLeads || dato.cantidad || dato.enviados || 1;
        ...
        const porcentajeCompletado = Math.min(
          100,
          (datosAcumulados / campana.cantidadDatosSolicitados) * 100,
        );
        const datosFaltantes = Math.max(
          0,
          campana.cantidadDatosSolicitados - datosAcumulados,
        );
        ...
        console.log(
          `Campaign ${campana.numeroCampana} mapping result:`,
          mapeoInfo,
        );
      ...
      res
        .status(500)
        .json({ error: "Failed to map campaigns with daily data" });
    }
  });
  ...
  app.get("/api/dashboard/datos-diarios-matching", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch datos diarios with matching" });
    }
  });
    ...
    try {
      ...
      const campanasConMatching = await Promise.all(
        campanas.map(async (campana) => {
          ...
          const fechaInicioObj = campana.fechaCampana
            ? new Date(campana.fechaCampana)
            : new Date();
          const hoy = new Date();
          ...
          const datosMatcheados = datosDiarios.filter((dato: any) => {
            ...
            const marcaCoincide =
              clienteNombre.includes(marcaBuscada) ||
          ...
          const porcentajeCompletado = Math.min(
            100,
            (datosObtenidos / campana.cantidadDatosSolicitados) * 100,
          );
          const datosFaltantes = Math.max(
            0,
            campana.cantidadDatosSolicitados - datosObtenidos,
          );
          ...
          return {
            ...
            ultimaFechaConDatos:
              datosMatcheados.length > 0
                ? datosMatcheados.sort(
                    (a, b) =>
  ...
  app.post("/api/dashboard/update-cpl", async (req, res) => {
    try {
      ...
      if (clienteNombre && numeroCampana) {
        await storage.updateCplByClienteAndCampana(
          clienteNombre,
          numeroCampana,
          cpl,
        );
        ...
        console.log(
          `CPL updated in database for client ${clienteIndex}: ${cpl}`,
        );
        ...
        res.status(400).json({
          error:
            "Must provide either clienteIndex or clienteNombre+numeroCampana",
        });
      }
    } catch (error) {
  ...
  app.post("/api/dashboard/update-venta", async (req, res) => {
    try {
      ...
      console.log("💰 Update venta request:", {
        clienteIndex,
        venta,
        clienteNombre,
        numeroCampana,
        ventaType: typeof venta,
      });
      ...
      if (isNaN(ventaNum) || ventaNum <= 0) {
        ...
        return res
          .status(400)
          .json({ error: "Venta must be a positive number" });
      }
      ...
      if (clienteNombre && numeroCampana) {
        try {
          ...
          console.log(
            `✅ Venta actualizada: ${clienteNombre} campaña ${numeroCampana} = $${ventaNum}`,
          );
        ...
        console.log(
          `✅ Venta actualizada por índice ${clienteIndex} = $${ventaNum}`,
        );
        ...
        res.status(400).json({
          error: "Debe proporcionar clienteNombre+numeroCampana o clienteIndex",
        });
      }
    } catch (error) {
  ...
  app.post("/api/dashboard/update-pedidos-por-dia", async (req, res) => {
    try {
      ...
      console.log(
        "⚡ Pedidos por día actualizado - invalidando cache dashboard",
      );
  ...
  app.post("/api/dashboard/force-refresh", async (req, res) => {
    try {
      console.log(
        '🔄 Forced refresh requested - clearing all caches and recalculating "enviados"',
      );
      ...
      try {
        ...
        const datosDiarios = await googleSheetsService.getDatosDiariosData();
        console.log(
          `📊 Procesando ${campanasComerciales.length} campañas con ${datosDiarios.length} registros de datos`,
        );
        ...
        for (const campana of campanasComerciales) {
          ...
          if (cliente) {
            ...
            console.log(
              `✓ Recalculando enviados para ${cliente.nombreCliente} - Campaña ${campana.numeroCampana}`,
            );
          }
        }

        console.log(
          `✅ Conteos "enviados" recalculados para ${campanasActualizadas} campañas`,
        );
      } catch (recalcError) {
        console.log(
          "⚠️ Error al recalcular conteos de enviados:",
          recalcError.message,
        );
      }
      ...
      res.json({
        success: true,
        message:
          'Dashboard actualizado exitosamente con recálculo de conteos "enviados"',
  ...
  app.post("/api/dashboard/update-enviados", async (req, res) => {
    try {
      console.log(
        '🔢 INICIANDO actualización específica de conteos "enviados"',
      );

      // Crear instancia del servicio especializado
      const updateEnviadosService = new UpdateEnviadosService(
        storage as any,
        googleSheetsService,
      );
      ...
      if (resultado.success) {
        console.log(
          `✅ Actualización exitosa: ${resultado.updated} campañas actualizadas`,
        );
        res.json({
          success: true,
  ...
  app.post("/api/data/sync-all", async (req, res) => {
    try {
      console.log(
        "🔄 CENTRALIZED: Iniciando sincronización completa database-first...",
      );
  ...
  app.get("/api/data/complete", async (req, res) => {
    try {
      console.log(
        "📊 CENTRALIZED: Obteniendo datos completos desde base de datos...",
      );
      ...
      const data =
        await centralizedDataService.getCompleteDataFromDatabase(filters);
  ...
  app.post("/api/dashboard/refresh-all-data", async (req, res) => {
    try {
      ...
      const { SyncFactory } = await import(
        "./sync/infrastructure/config/SyncFactory"
      );
      ...
      console.log(
        `✅ Sistema refactorizado procesó ${syncResult.leadsProcessed} leads`,
      );
      ...
      try {
        ...
        const metaResponse = await fetch(
          "http://localhost:5000/api/meta-ads/sync",
          {
            method: "POST",
      ...
      console.log(
        '🔄 Forzando recálculo de conteos de "enviados" por campaña...',
      );
      try {
        // Llamar internamente al endpoint datos-diarios para forzar recálculo
        const datosRecalculados = await fetch(
          "http://localhost:5000/api/dashboard/datos-diarios",
        );
        if (datosRecalculados.ok) {
          const datosJson = await datosRecalculados.json();
          console.log(
            `✅ Recalculados conteos de "enviados" para ${datosJson.length} campañas`,
          );
        } else {
        ...
        console.log(
          "⚠️ No se pudo forzar recálculo de conteos:",
          recalcError.message,
        );
      }

      console.log(
        `✅ Actualización completa terminada. Procesadas ${processedCount} campañas`,
      );
      res.json({
        success: true,
        message:
          'Datos actualizados completamente incluye recálculo de "enviados"',
      ...
      res
        .status(500)
        .json({ error: `Error al actualizar datos: ${error.message}` });
    }
  });
  ...
  app.get("/api/finanzas/centralized", async (req, res) => {
    try {
      console.log(
        "💰 CENTRALIZED FINANZAS: Obteniendo datos financieros desde base de datos...",
      );
      ...
      console.log(
        `💰 CENTRALIZED FINANZAS: ${finanzasData.length} registros financieros desde BD`,
      );
      ...
      res
        .status(500)
        .json({ error: `Error obteniendo finanzas: ${error.message}` });
    }
  });
  ...
  app.get("/api/datos-diarios/centralized", async (req, res) => {
    try {
      console.log(
        "📊 CENTRALIZED DATOS DIARIOS: Obteniendo desde base de datos...",
      );
      ...
      const datosDiariosData = data.map((item) => ({
        ...
        inversionRealizada: item.inversion,
        inversionPendiente: Math.max(
          0,
          item.cantidadDatosSolicitados * item.cpl - item.inversion,
        ),
        porcentajeCompletado:
          item.cantidadDatosSolicitados > 0
            ? Math.round((item.enviados / item.cantidadDatosSolicitados) * 100)
            : 0,
        estado:
          item.enviados >= item.cantidadDatosSolicitados
            ? "Completada"
            : "En Progreso",
        cpa: item.cpa,
      ...
      console.log(
        `📊 CENTRALIZED DATOS DIARIOS: ${datosDiariosData.length} registros desde BD`,
      );
      ...
      res
        .status(500)
        .json({ error: `Error obteniendo datos diarios: ${error.message}` });
    }
  });
  ...
  app.get("/api/dashboard/finanzas", async (req, res) => {
    try {
      ...
      const datosDiariosCompletos = await fetch(
        "http://localhost:5000/api/dashboard/datos-diarios",
      );
      ...
      const finanzasData = await Promise.all(
        datosDiarios.map(async (data: any, index: number) => {
          ...
          if (
            clienteNombre.includes("fiat") ||
            clienteCompleto.includes("fiat")
          )
            marca = "Fiat";
          else if (
            clienteNombre.includes("peugeot") ||
            clienteCompleto.includes("peugeot")
          )
            marca = "Peugeot";
          else if (
            clienteNombre.includes("toyota") ||
            clienteCompleto.includes("toyota")
          )
            marca = "Toyota";
          else if (
            clienteNombre.includes("chevrolet") ||
            clienteCompleto.includes("chevrolet")
          )
            marca = "Chevrolet";
          else if (
            clienteNombre.includes("renault") ||
            clienteCompleto.includes("renault")
          )
            marca = "Renault";
          else if (
            clienteNombre.includes("citroen") ||
            clienteCompleto.includes("citroen")
          )
            marca = "Citroen";
          ...
          const campanaComercial = campanasComerciales.find(
            (c: any) =>
          ...
          if (!cplValue) {
            switch (marca) {
              ...
              case "Citroen":
                ...
                break;
              default:
                cplValue = parseFloat(data.cpl) || 0;
            }
          }
          ...
          if (
            campanaComercial?.facturacionBruta &&
            parseFloat(campanaComercial.facturacionBruta.toString()) > 0
          ) {
            // Usar facturación bruta de la campaña comercial si está disponible y es mayor a 0
            totalFacturado = parseFloat(
              campanaComercial.facturacionBruta.toString(),
            );
            console.log(
              `💰 FACTURACIÓN BRUTA MANUAL: ${data.clienteNombre} = $${totalFacturado.toFixed(2)}`,
            );
          } else {
            // Calcular automáticamente: Leads × CPL × Venta por Campaña
            totalFacturado = leadCount * cplValue * ventaPorCampana;
            console.log(
              `💰 FACTURACIÓN BRUTA CALCULADA: ${data.clienteNombre} = ${leadCount} leads × $${cplValue} CPL × ${ventaPorCampana} venta = $${totalFacturado.toFixed(2)}`,
            );
          }
          ...
          if (marca === "Peugeot" || data.clienteNombre?.includes("ALBENS")) {
            console.log(
              `🔍 PEUGEOT DEBUG: ${data.clienteNombre} #${data.numeroCampana} = ${leadCount} leads, CPL=${cplValue}, Venta=${ventaPorCampana}, Facturado=$${totalFacturado.toFixed(2)}`,
            );
          }
          ...
          try {
            ...
            if (metaAdsService && leadCount > 0) {
              ...
              console.log(
                `🔍 CPA FINANZAS: ${marca} | Gasto: $${inversionMetaAds} | Leads: ${leadCount} | CPA: $${cpaValue.toFixed(2)}`,
              );
            }
          } catch (error) {
            console.error(
              `Error calculating CPA for finanzas ${data.clienteNombre}:`,
              error,
            );
            cpaValue = 0;
            inversionMetaAds = 0;
          ...
          const inversionCalculada =
            inversionMetaAds > 0
              ? inversionMetaAds * 1.02
              : cplValue * leadCount;

          console.log(
            `💰 INVERSIÓN ${data.clienteNombre} (${marca}): Meta Ads $${inversionMetaAds} + 2% = $${inversionCalculada.toFixed(2)}`,
          );

          // Calcular ganancia y ROI usando totalFacturado
          const ganancia = totalFacturado - inversionCalculada;
          const roi =
            inversionCalculada > 0 ? (ganancia / inversionCalculada) * 100 : 0;
          ...
          return {
            ...
            fechaCampana:
              ...
              data.fecha ||
              data.fechaInicio ||
              new Date().toISOString().split("T")[0], // Agregar fecha para filtro de mes
          };
        }),
      );
      ...
      Object.entries(resumenPorMarca).forEach(
        ([marca, datos]: [string, any]) => {
          console.log(
            `   ${marca}: ${datos.campanasCount} campañas, ${datos.totalLeads} leads totales, $${datos.totalFacturado.toFixed(2)} facturado total`,
          );
        },
      );

      console.log(
        `📊 Finanzas: Mapeadas ${finanzasData.length} campañas con inversión real por campaña`,
      );
      res.json(finanzasData);
    } catch (error) {
  ...
  app.get("/api/sheets/clientes", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch clientes from Google Sheets" });
    }
  });
  ...
  app.post("/api/client-matching/test", async (req, res) => {
    try {
      ...
      if (!clienteName || !dataName) {
        return res
          .status(400)
          .json({ error: "clienteName and dataName are required" });
      }

      // Usar el nuevo sistema refactorizado
      const clientMatcher = getClientMatcher();
      const isMatch = clientMatcher
        ? clientMatcher.isMatch(clienteName, dataName)
        : false;
      const matchingRule = clientMatcher
        ? clientMatcher.findMatchingRule(clienteName, dataName)
        : null;
  ...
  app.post("/api/campanas-comerciales", async (req, res) => {
    try {
      ...
      const { fechaFin, numeroCampana, ...dataWithoutCalculatedFields } =
        req.body;
      ...
      if (dataWithoutCalculatedFields.fechaCampana) {
        ...
        console.log(
          "Fecha mantenida sin conversión timezone:",
          dataWithoutCalculatedFields.fechaCampana,
        );
      }

      const validatedData = createCampanaComercialSchema.parse(
        dataWithoutCalculatedFields,
      );

      // Generar número de campaña automáticamente basado en cliente
      const existingCampanas = await storage.getCampanasPorCliente(
      ...
      console.log(
        "Nueva campaña SIN fecha fin inicial - se calculará automáticamente al completarse",
      );
      ...
      console.log(
        "🔄 Broadcasting campaign creation update to frontend clients",
      );
  ...
  app.put("/api/campanas-comerciales/:id", async (req, res) => {
    try {
      ...
      console.log(
        "⚡ Campaña actualizada - invalidando cache del dashboard datos-diarios",
      );
  ...
  app.put("/api/campanas-comerciales/:id/reopen", async (req, res) => {
    try {
      ...
      const campana = await storage.updateCampanaComercial(id, {
        fechaFin: null,
      });
      ...
      console.log(
        `✅ Campaña ${id} reabierta completamente - fechaFin eliminada y leads liberados`,
      );

      // CRÍTICO: Notificar al frontend para actualización inmediata
      console.log(
        "⚡ Campaña reabierta - invalidando cache del dashboard datos-diarios",
      );
  ...
  app.put("/api/campanas-comerciales/:id/pedidos-por-dia", async (req, res) => {
    try {
      ...
      if (typeof pedidosPorDia !== "number" || pedidosPorDia < 0) {
        return res.status(400).json({
          error: "pedidosPorDia debe ser un número mayor o igual a 0",
        });
      }
      ...
      // CRÍTICO: Invalidar cache para actualización inmediata en datos-diarios
      console.log(
        `⚡ Pedidos por día actualizado para campaña ${id}: ${pedidosPorDia} - invalidando cache dashboard`,
      );
  ...
  app.delete("/api/campanas-comerciales/:id", async (req, res) => {
    try {
      ...
      console.log(
        `🔄 Números de campaña recalculados para cliente ${clienteId}`,
      );

      // CRÍTICO: Invalidar cache para actualización inmediata en datos-diarios
      console.log(
        "⚡ Campaña eliminada - invalidando cache del dashboard datos-diarios",
      );
  ...
  app.get("/api/campanas-comerciales/by-client-campaign", async (req, res) => {
    try {
      ...
      console.log("🔍 Buscando campaña real:", {
        clienteNombre,
        numeroCampana,
      });
      ...
      if (!clienteNombre || !numeroCampana) {
        return res
          .status(400)
          .json({ error: "clienteNombre and numeroCampana are required" });
      }
      ...
      console.log(
        `📊 Total campañas en BD: ${campanas.length}, Total clientes: ${clientes.length}`,
      );
      ...
      const cliente = clientes.find(
        (c) =>
          c.nombreComercial
            .toLowerCase()
            .includes((clienteNombre as string).toLowerCase()) ||
          c.nombreCliente
            .toLowerCase()
            .includes((clienteNombre as string).toLowerCase()) ||
          (clienteNombre as string)
            .toLowerCase()
            .includes(c.nombreComercial.toLowerCase()) ||
          (clienteNombre as string)
            .toLowerCase()
            .includes(c.nombreCliente.toLowerCase()),
      );

      console.log(
        "👤 Cliente encontrado:",
        cliente
          ? `${cliente.nombreComercial} (ID: ${cliente.id})`
          : "No encontrado",
      );

      if (!cliente) {
        console.log(
          "❌ Cliente no encontrado. Clientes disponibles:",
          clientes.slice(0, 5).map((c) => c.nombreComercial),
        );
        return res.status(404).json({ error: "Cliente not found" });
      }
      ...
      console.log(
        "🎯 Campaña encontrada:",
        campana
          ? `${campana.numeroCampana} (ID: ${campana.id})`
          : "No encontrada",
      );
      ...
      if (!campana) {
        ...
        console.log(
          "❌ Campaña no encontrada. Campañas del cliente:",
          campanasDelCliente.map((c) => c.numeroCampana),
        );
        return res.status(404).json({ error: "Campaña not found" });
      }

      console.log("✅ Resultado exitoso con datos reales de BD:", {
        id: campana.id,
      });
  ...
  app.get("/api/leads/duplicates", (req, res) => {
    try {
      ...
      console.log(
        `🔍 Detecting duplicates for cliente: ${cliente}, campaña: ${campaña}`,
      );
      ...
      if (cliente) {
        ...
        for (const [key, count] of Object.entries(duplicatesByClient)) {
          if (
            clienteName.includes(key.toUpperCase()) ||
            key.toUpperCase().includes(clienteName)
          ) {
      ...
      console.log(
        `✅ Found ${duplicateCount} duplicate phone numbers for ${cliente || "all clients"}`,
      );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `🎯 Calculando fecha exacta para ${marca}, ${cantidadSolicitada} leads desde ${fechaInicio}`,
        );

        const fechaFinExacta = await calculateFechaFin(
          fechaInicio,
          cantidadSolicitada,
          marca,
        );
  ...
  async function calculateFechaFin(
    fechaInicio: string,
    cantidadSolicitada: number,
    marca: string,
  ): Promise<string> {
    try {
      console.log("🔢 Calculando fecha_fin automática:", {
        fechaInicio,
        cantidadSolicitada,
        marca,
      });
      ...
      console.log(
        `📊 Encontrados ${leadsResult.length} leads de ${marca} desde ${fechaInicio}`,
      );
      ...
      if (leadsResult.length === 0) {
        ...
        fechaEstimada.setDate(
          fechaEstimada.getDate() + Math.ceil(cantidadSolicitada / 3),
        ); // Estimación: 3 leads por día
        fechaEstimada.setHours(23, 59, 59); // Fin del día estimado
        const fechaFinalConHora = fechaEstimada
          .toISOString()
          .substring(0, 19)
          .replace("T", " ");
        console.log(
          `⚠️ Sin datos disponibles - fecha estimada: ${fechaFinalConHora}`,
        );
      ...
      const leadsOrdenados = leadsResult.sort(
        (a, b) => a.leadDate.getTime() - b.leadDate.getTime(),
      );
      ...
      for (let i = 0; i < leadsOrdenados.length; i++) {
        ...
        const fechaHoraCompleta = timestampCompleto
          .substring(0, 19)
          .replace("T", " "); // "2025-08-15 14:30:45"

        console.log(
          `🎯 Lead #${contador}: ${fechaHoraCompleta} - Campaign: ${leadActual.campaignName}`,
        );
        ...
        if (contador >= cantidadSolicitada) {
          ...
          console.log(
            `⏰ FECHA EXACTA ALCANZADA: Lead #${cantidadSolicitada} en ${fechaHoraCompleta}`,
          );
          break;
        }
      }
      ...
      fechaInicio.setDate(
        fechaInicio.getDate() + Math.ceil(cantidadSolicitada / 2),
      );
  ...
  app.get("/api/sheets/auto-detect", async (req, res) => {
    try {
      ...
      const availableSheets =
        await googleSheetsService.getAvailableSheetNames();
      ...
      res.json({
        ...
        sheetsInfo,
        totalLeads: sheetsInfo.reduce(
          (sum, sheet) => sum + sheet.leadsCount,
          0,
        ),
      });
    } catch (error) {
  ...
  app.get("/api/enviados/database/:cliente", async (req, res) => {
    try {
      ...
      const count = await storage.getEnviadosFromDatabase(
        cliente,
        marca as string,
      );
  ...
  app.post("/api/sync/incremental", async (req, res) => {
    try {
      ...
      const options = {
        ...
        specificSheets: req.query.sheets
          ? (req.query.sheets as string).split(",")
          : undefined,
      };
      ...
      res.json({
        ...
        message: result.success
          ? `Sincronización incremental: ${result.leadsProcessed} leads procesados`
  ...
  app.get("/api/sync/status", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Error obteniendo estado de sincronización" });
    }
  });
  ...
  app.get("/api/op-leads/clientes", async (req, res) => {
    try {
      ...
      console.log(
        "📊 Obteniendo listado de clientes desde vista_clientes_op_leads...",
      );
  ...
  app.get("/api/cpl-analysis", async (req, res) => {
    try {
      ...
      if (!dateFrom || !dateTo) {
        return res.status(400).json({
          error:
            "Parámetros dateFrom y dateTo son requeridos (formato YYYY-MM-DD)",
        });
      }

      console.log(
        `🔍 Análisis CPL iniciado para rango: ${dateFrom} - ${dateTo}`,
      );
      ...
      if (!metaAdsService) {
        return res.status(400).json({
          error:
            "Meta Ads service not configured. Please configure Meta Ads first.",
        });
      }
      ...
      metaAdsData.forEach((campaign) => {
        ...
        const marcaEncontrada = marcas.find(
          (marca) =>
            nombreCampana.includes(marca) ||
            nombreCampana.includes(marca.toLowerCase()),
        );
      ...
      Object.keys(marcasMetaAds).forEach((marca) => {
        ...
        data.cplMetaAds =
          data.leadsMetaAds > 0 ? data.importeGastado / data.leadsMetaAds : 0;
      });

      console.log(
        `🎯 Marcas procesadas de Meta Ads: ${Object.keys(marcasMetaAds).join(", ")}`,
      );
      ...
      for (const marca of Object.keys(marcasMetaAds)) {
        ...
        // Calcular CPL Real
        const cplReal =
          leadsReales > 0 ? metaData.importeGastado / leadsReales : 0;
        ...
        resultadoFinal.push({
          ...
          diferenciaCPL:
            Math.round((metaData.cplMetaAds - cplReal) * 100) / 100,
          diferenciaPorcentaje:
            metaData.cplMetaAds > 0
              ? Math.round(
                  ((metaData.cplMetaAds - cplReal) / metaData.cplMetaAds) *
                    100 *
                    100,
                ) / 100
              : 0,
        ...
        console.log(
          `✅ ${marca}: Meta=${metaData.leadsMetaAds} leads, Real=${leadsReales} leads, CPL Meta=${metaData.cplMetaAds.toFixed(2)}, CPL Real=${cplReal.toFixed(2)}`,
        );
      }
      ...
      res.json({
        ...
        summary: {
          ...
          totalImporteGastado:
            Math.round(
              resultadoFinal.reduce(
                (sum, item) => sum + item.importeGastado,
                0,
              ) * 100,
            ) / 100,
          totalLeadsMetaAds: resultadoFinal.reduce(
            (sum, item) => sum + item.leadsMetaAds,
            0,
          ),
          totalLeadsReales: resultadoFinal.reduce(
            (sum, item) => sum + item.leadsReales,
            0,
          ),
        },
        timestamp: new Date(),
  ...
  app.get("/api/finanzas-meta-ads", async (req, res) => {
    try {
      ...
      console.log(
        `💰 Análisis Financiero Meta Ads iniciado para rango: ${dateFrom} - ${dateTo}`,
      );
      ...
      console.log(
        `📊 Meta Ads Finanzas: ${metaAdsData.length} campañas obtenidas`,
      );

      // 3. Agrupar campañas Meta Ads por marca
      const marcasFinanzas: Record<
        string,
        {
          importeGastado: number;
          leadsMetaAds: number;
      ...
      console.log(
        `🎯 Marcas procesadas para Finanzas: ${Object.keys(marcasFinanzas).join(", ")}`,
      );
      ...
      for (const marca of Object.keys(marcasFinanzas)) {
        ...
        const campanasDelaMarca = campanasComerciales.filter(
          (campana: any) =>
        ...
        campanasDelaMarca.forEach((campana: any) => {
          if (
            campana.facturacionBruta &&
            parseFloat(campana.facturacionBruta.toString()) > 0
          ) {
            facturacionBrutaReal += parseFloat(
              campana.facturacionBruta.toString(),
            );
          }
        });

        console.log(
          `💰 ${marca}: Encontradas ${campanasDelaMarca.length} campañas, Facturación real: $${facturacionBrutaReal}`,
        );

        // Configuración por marca (puedes ajustar estos valores)
        const configPorMarca: Record<
          string,
          { ventaPromedio: number; comisionPorcentaje: number }
        > = {
          PEUGEOT: { ventaPromedio: 0.12, comisionPorcentaje: 15 },
          FIAT: { ventaPromedio: 0.1, comisionPorcentaje: 12 },
          ...
          CITROEN: { ventaPromedio: 0.1, comisionPorcentaje: 12 },
        ...
        const config = configPorMarca[marca] || {
          ventaPromedio: 0.12,
        ...
        if (facturacionBruta === 0) {
          ...
          console.log(
            `💡 ${marca}: Usando facturación calculada $${facturacionBruta.toFixed(2)} (sin datos reales)`,
          );
        } else {
          console.log(
            `✅ ${marca}: Usando facturación REAL $${facturacionBruta.toFixed(2)} de campañas comerciales`,
          );
        }
        ...
        const diferencPorcentajeLeads =
          metaData.leadsMetaAds > 0
            ? Math.round(
                ((metaData.leadsMetaAds - leadsReales) /
                  metaData.leadsMetaAds) *
                  100 *
                  100,
              ) / 100
            : 0;
        ...
        console.log(
          `💰 ${marca}: Meta=${metaData.leadsMetaAds} leads, Real=${leadsReales} leads, Inversión Meta=$${inversionMetaAdsPura.toFixed(2)}, Impuestos Meta Ads=$${impuestosMetaAds.toFixed(2)}, Total=$${inversionTotal.toFixed(2)}, Facturación=$${facturacionBruta.toFixed(2)}, Impuestos=$${totalImpuestos.toFixed(2)}, Ganancia=$${ganancia.toFixed(2)}, ROI=${roi.toFixed(2)}%`,
        );
      }
      ...
      res.json({
        ...
        summary: {
          ...
          totalLeadsMetaAds: resultadoFinanciero.reduce(
            (sum, item) => sum + item.leadsMetaAds,
            0,
          ),
          totalLeadsReales: resultadoFinanciero.reduce(
            (sum, item) => sum + item.leadsReales,
            0,
          ),
          totalInversionMetaAds:
            Math.round(
              resultadoFinanciero.reduce(
                (sum, item) => sum + item.inversionMetaAds,
                0,
              ) * 100,
            ) / 100,
          totalImpuestosMetaAds:
            Math.round(
              resultadoFinanciero.reduce(
                (sum, item) => sum + item.impuestosMetaAds,
                0,
              ) * 100,
            ) / 100,
          totalInversionTotal:
            Math.round(
              resultadoFinanciero.reduce(
                (sum, item) => sum + item.inversionTotal,
                0,
              ) * 100,
            ) / 100,
          totalFacturacion:
            Math.round(
              resultadoFinanciero.reduce(
                (sum, item) => sum + item.facturacionBruta,
                0,
              ) * 100,
            ) / 100,
          totalIIBB:
            Math.round(
              resultadoFinanciero.reduce((sum, item) => sum + item.iibb, 0) *
                100,
            ) / 100,
          totalIVA:
            Math.round(
              resultadoFinanciero.reduce((sum, item) => sum + item.iva, 0) *
                100,
            ) / 100,
          totalImpuestos:
            Math.round(
              resultadoFinanciero.reduce(
                (sum, item) => sum + item.totalImpuestos,
                0,
              ) * 100,
            ) / 100,
          totalGanancia:
          ...
          roiPromedio:
            resultadoFinanciero.length > 0
              ? Math.round(
                  (resultadoFinanciero.reduce(
                    (sum, item) => sum + item.roi,
                    0,
                  ) /
                    resultadoFinanciero.length) *
                    100,
                ) / 100
              : 0,
  ...
  try {
    console.log(
      "🔄 Registrando rutas del sistema de sincronización refactorizado...",
    );
    const { createSyncRoutes, syncLoggingMiddleware } = await import(
      "./sync/presentation/routes/sync-routes"
    );
  ...
  try {
    ...
    const { createCampaignClosureRoutes, campaignClosureLoggingMiddleware } =
      await import(
        "./campaign-closure/presentation/routes/campaign-closure-routes"
      );
    ...
    console.log(
      "✅ Rutas del sistema de cierre de campañas registradas: /api/campaign-closure/*",
    );
  } catch (error) {
    console.error(
      "❌ Error registrando rutas del sistema refactorizado:",
      error,
    );
  }
  ...
  try {
    ...
    const { createCampaignResetRoutes } = await import(
      "./campaign-reset/presentation/routes/campaign-reset-routes"
    );
  ...
  try {
    ...
    const { createPendingCampaignRoutes, pendingCampaignLoggingMiddleware } =
      await import(
        "./pending-campaigns/presentation/routes/pending-campaign-routes"
      );
    ...
    console.log(
      "✅ Rutas del sistema de campañas pendientes registradas: /api/pending-campaigns/*",
    );
  } catch (error) {
    console.error(
      "❌ Error registrando rutas del sistema de campañas pendientes:",
      error,
    );
  }
  ...
  try {
    ...
    const { createFinishedCampaignRoutes, finishedCampaignLoggingMiddleware } =
      await import(
        "./finished-campaigns/presentation/routes/finished-campaign-routes"
      );
    ...
    console.log(
      "✅ Rutas del sistema de campañas finalizadas registradas: /api/finished-campaigns/*",
    );
  } catch (error) {
    console.error(
      "❌ Error registrando rutas del sistema de campañas finalizadas:",
      error,
    );
  }
  ...
  try {
    const { leadsRoutes } = await import(
      "./leads/presentation/routes/leads-routes"
    );
