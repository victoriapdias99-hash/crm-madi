...
export class PostgresSyncRepository implements ISyncRepository {
  ...
  async createLead(lead: ProcessedSyncLead): Promise<SyncLead> {
    try {
      // Insertando lead en la base de datos
  ...
  async findLeadByMetaId(metaLeadId: string): Promise<SyncLead | null> {
    ...
    try {
      const leads = await this.storage.getLeads({ limit: 10000 });
      const matchingLead = leads.find(
        (lead: any) => lead.metaLeadId === metaLeadId,
      );
      ...
      for (let i = 0; i < leads.length; i += batchSize) {
        ...
        for (const lead of batch) {
          try {
            ...
            console.warn(
              `Failed to create lead ${lead.metaLeadId}:`,
              error.message,
            );
          }
        }
  ...
  async findDuplicatesByMetaId(metaIds: string[]): Promise<SyncLead[]> {
    try {
      ...
      const duplicates = await db
        ...
        .from(opLead)
        .where(
          sql`${opLead.metaLeadId} IN ${sql.raw(`(${metaIds.map((id) => `'${id}'`).join(", ")})`)}`,
        );
   ...
   */
  async findDuplicatesByRowNumber(
    marca: string,
    googleSheetsRowNumbers: number[],
  ): Promise<SyncLead[]> {
    try {
      ...
      const duplicates = await db
        ...
        .from(opLead)
        .where(
          sql`${opLead.marca} = ${marca.toUpperCase()} AND ${opLead.googleSheetsRowNumber} IN ${sql.raw(`(${googleSheetsRowNumbers.join(", ")})`)}`,
        );

      console.log(
        `🔍 Encontrados ${duplicates.length} duplicados por fila para marca ${marca}: filas [${googleSheetsRowNumbers.join(", ")}]`,
      );
      return duplicates.map(this.mapOpLeadToSyncLead);
    } catch (error) {
      console.error(
        `Error finding duplicates by row number for ${marca}:`,
        error,
      );
      return [];
    }
  ...
  async getLastProcessedRowByBrand(marca: string): Promise<number> {
    try {
      ...
      const result = await db
        .select({ maxRow: sql<number>`MAX(${opLead.googleSheetsRowNumber})` })
        .from(opLead)
        .where(
          sql`${opLead.marca} = ${marca.toUpperCase()} AND ${opLead.googleSheetsRowNumber} IS NOT NULL`,
        );
      ...
      console.error(
        `Error getting last processed row for brand ${marca}:`,
        error,
      );
      return 0;
    }
  ...
  async cleanDuplicatesForBrand(marca: string): Promise<number> {
    try {
      ...
      const duplicateRows = await db
        ...
        .from(opLead)
        .where(
          sql`${opLead.marca} = ${normalizedBrand} AND ${opLead.googleSheetsRowNumber} IS NOT NULL`,
        )
      ...
      console.log(
        `🧽 ${marca}: Encontrados ${duplicateRows.length} grupos de duplicados`,
      );
      ...
      for (const duplicateGroup of duplicateRows) {
        ...
        const groupRecords = await db
          ...
          .from(opLead)
          .where(
            sql`${opLead.marca} = ${normalizedBrand} AND ${opLead.googleSheetsRowNumber} = ${googleSheetsRowNumber}`,
          )
        ...
        if (idsToDelete.length > 0) {
          const deleteResult = await db
            .delete(opLead)
            .where(
              sql`${opLead.id} IN ${sql.raw(`(${idsToDelete.join(", ")})`)}`,
            )
          ...
          console.log(
            `🗑️ ${marca} fila ${googleSheetsRowNumber}: Eliminados ${deletedCount} duplicados, mantenido ID ${groupRecords[0].id}`,
          );
        }
      }

      console.log(
        `✨ ${marca}: Limpieza completada - ${totalDeleted} duplicados eliminados`,
      );
      return totalDeleted;
    } catch (error) {
  ...
  async getExistingLeadsByBrand(marca: string): Promise<SyncLead[]> {
    try {
      ...
      console.log(
        `📊 Encontrados ${existingLeads.length} leads existentes con googleSheetsRowNumber para marca ${marca}`,
      );
  ...
  async getGoogleSheetsRowNumbers(marca: string): Promise<number[]> {
    try {
      ...
      console.log(
        `🔍 ${marca}: ${rowNumbers.length} filas en BD, rango: ${rowNumbers[0] || 0}-${rowNumbers[rowNumbers.length - 1] || 0}`,
      );
      return rowNumbers;
    } catch (error) {
      console.error(
        `Error getting Google Sheets row numbers for ${marca}:`,
        error,
      );
      return [];
    }
  ...
  private mapStorageLeadToSyncLead(storageLead: any): SyncLead {
    return {
      ...
      fechaCreacion: storageLead.leadDate
        ? storageLead.leadDate.toISOString()
